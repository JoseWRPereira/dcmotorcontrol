...
//------------UART_Init_80MHz_115200bps------------
// Initialize the UART for 115200 baud rate (assuming 80 MHz UART clock),
// 8 bit word length, no parity bits, one stop bit, FIFOs enabled
// Input: none
// Output: none
void initUART2_80MHz_9600bps(void)
{
  SYSCTL_RCGC1_R |= SYSCTL_RCGC1_UART2; // activate UART2
  SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOD; // activate port D
  UART2_CTL_R &= ~UART_CTL_UARTEN;      // disable UART
  UART2_IBRD_R = 520;   // IBRD = int(80,000,000 / (16 * 9600)) = int(520.83)
  UART2_FBRD_R = 53;    // FBRD = int(0.833 * 64 + 0.5) = 53.833
//   UART2_IBRD_R = 43;// IBRD = int(80,000,000 / (16 * 115200)) = int(43.403)
//  UART2_FBRD_R = 26; // FBRD = int(0.403 * 64 + 0.5) = 26.292
  	// 8 bit word length (no parity bits, one stop bit, FIFOs)
  UART2_LCRH_R = (UART_LCRH_WLEN_8|UART_LCRH_FEN);
  UART2_CTL_R |= (UART_CTL_UARTEN);     // enable UART
