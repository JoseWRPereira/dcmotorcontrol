...
void initUART0_80MHz_115200bps(void)
{
  SYSCTL_RCGC1_R |= SYSCTL_RCGC1_UART0; // activate UART0
  SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOA; // activate port A
  UART0_CTL_R &= ~UART_CTL_UARTEN;      // disable UART
  UART0_IBRD_R = 43;                    // IBRD = int(80,000,000 / (16 * 115,200)) = int(43.403)
  UART0_FBRD_R = 26;                    // FBRD = int(0.403 * 64 + 0.5) = 26
                                        // 8 bit word length (no parity bits, one stop bit, FIFOs)
  UART0_LCRH_R = (UART_LCRH_WLEN_8|UART_LCRH_FEN);
  UART0_CTL_R |= UART_CTL_UARTEN;       // enable UART
  GPIO_PORTA_AFSEL_R |= 0x03;           // enable alt funct on PA1-0
...
void initUART0_16MHz_115200bps( void )// UART0 115 200 baud rate (clock 16MHz)
{
//------------UART_Init------------
// Initialize the UART for 115,200 baud rate (assuming 16 MHz UART clock),
// 8 bit word length, no parity bits, one stop bit, FIFOs enabled
...
// Output: none
  ...
  UART0_IBRD_R = 8;  // IBRD = int(16,000,000 / (16 * 115,200)) = int(8.6805)
  UART0_FBRD_R = 44; // FBRD = int(0.6805 * 64 + 0.5) = 44
	             // 8 bit word length (noParityBits, 1stopBit, FIFOs)
  UART0_LCRH_R = 0x0070;
  UART0_CTL_R |= 0x0301;	// enable UART
  GPIO_PORTA_AMSEL_R &= ~0x03;  // disable analog functionality on PA
  GPIO_PORTA_AFSEL_R |= 0x03;   // enable alt funct on PA1-0
